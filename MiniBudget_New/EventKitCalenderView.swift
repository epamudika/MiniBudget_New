...
struct EventKitCalendarView: View {
    ...
    // Nav bar
    // "← Home" back button — left-aligned, dark text.
    ...
    // Calendar Sync row ───────────────────────────────
    // White card: red calendar icon + "Calendar Sync" + subtitle.
    // A small coloured dot on the right shows sync status.
    private var calendarSyncRow: some View {
        HStack(spacing: 14) {
            ...
            // Sync status dot: green = access granted, grey = not
            Circle()
    ...
    // Day-of-week headers
    // SUN MON TUE WED THU FRI SAT — small grey tracking text.
    ...
    // Bottom stats row ────────────────────────────────
    // Two mint-green cards: Daily Avg and Current Streak.
    ...
    // Requests write-only calendar access (iOS 17+) or full access
    // (iOS 16 and below) using the EventKit framework.
    private func requestAccess() {
        if #available(iOS 17.0, *) {
            ...
            // iOS 16 and below: full event access
    ...
    // Creates or skips an all-day EKEvent in "Mini Budget" calendar.
    //Duplicate check prevents writing the same day twice.
    ...
    private func syncDay(_ day: Int) {
        ...
        ev.isAllDay  = true
        ev.startDate = start
        ev.endDate   = start       // All-day event: start == end
        ev.calendar  = cal
    ...
    // Returns the existing "Mini Budget" EKCalendar, or creates
    // a new green-coloured one if it doesn't exist yet.
    private func minibudgetCalendar() -> EKCalendar {
        ...
        // Assign to the default source (iCloud or local)
        cal.source = store.defaultCalendarForNewEvents?.source
            ?? store.sources.first { $0.sourceType == .local }
            ?? store.sources.first!
        // Brand green colour for the calendar dot in Apple Calendar
        cal.cgColor = UIColor(red: 0.11, green: 0.73, blue: 0.33, alpha: 1).cgColor
        try? store.saveCalendar(cal, commit: true)
        return cal
    }

    // Opens iOS Settings so the user can grant calendar permission.
    ...
    // "March 2026" formatted string for the header.
...
struct CalendarDayCell: View {
    ...
    var body: some View {
        VStack(spacing: 2) {
            ...
            // Day number — wrapped in ZStack for the today ring
            ZStack {
                if isToday {
                    Circle()
            ...
            // Green dot — visible only on saved days
            Circle()
                .fill(dotColor)
