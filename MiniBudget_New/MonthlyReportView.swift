...
struct MonthlyReportView: View {
    ...
    // Data
    // In production these come from a Core Data NSFetchRequest
    ...
    // Weekly bar chart data — label + amount + relative height (0.0–1.0)
    // Heights are visual proportions, not the raw amounts,
    // so the chart looks good regardless of data scale.
    private let weeklyBars: [(label: String, amount: Double, height: CGFloat)] = [
        ("WEEK 1", 400, 0.50),
        ("WEEK 2", 600, 0.70),
        ("WEEK 3", 750, 0.85),
        ("TODAY",  900, 1.00),  // Tallest bar = darkest green
    ]
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: 12) {
                        ...
                        // 3 & 4. Total Savings label + amount
                        totalSavingsSection
                    ...
                    VStack(spacing: 0) {
                        ...
                        Spacer().frame(height: 100) // clears tab bar
                    }
    ...
    // Page title
    // "March Report" centred, green, bold.
    ...
    // Returns the correct green shade based on the bar's height fraction.
    // Taller bar = darker green (matches the wireframe gradient effect).
    private func barColor(for height: CGFloat) -> Color {
        switch height {
        case 0.9...:  return Color(hex: "#1B5E20")   // Darkest — TODAY
        case 0.7...:   return Color(hex: "#2E7D32")   // Dark
        case 0.5...:   return Color(hex: "#4CAF50")   // Medium
        default:      return Color(hex: "#A5D6A7")   // Light
        }
    }

    // Stats row
    // Two white cards: Best Day and Total Badges.
...
private struct ReportTabBar: View {
    ...
    private let tabs: [(icon: String, label: String, active: Bool)] = [
        ("house",      "Home",     false),
        ("chart.bar",  "History",  true ),  // ← Active on Report screen
