...
import SwiftUI
import UserNotifications   // For requesting notification permission


// Data Model

// Represents one notification item in the Recent Activity list.
...
struct PushNotificationView: View {
    ...
    // In production save this to @AppStorage or Core Data.
    ...
    // In production these come from a NotificationManager / Core Data.
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                // Notification list
                // Each item is rendered by NotificationRow view.
                VStack(spacing: 10) {
    ...
    private var calendarSyncRow: some View {
        HStack(spacing: 14) {
            ...
            //iOS-style Toggle
            // .tint sets the ON colour to brand green.
    ...
    // Asks the system for notification permission when Calendar Sync
    // is toggled on.  Required by iOS before scheduling any alerts.
...
struct NotificationRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            //  Icon circle
            // Coloured background circle + SF Symbol icon inside.
            ZStack {
                Circle()
        ...
        .background(Color.white)
        .cornerRadius(14)
        //  Red left border accent for alert notifications
        // The "Smart Decision" row has isAlert = true, which adds
        // a 3pt red rectangle on the left edge of the card.
        .overlay(
            Group {
                if item.isAlert {
                    HStack {
