...
/// Returns a fresh in-memory PersistenceController for every test.
private func makeInMemoryController() -> PersistenceController {
    PersistenceController(inMemory: true)
}

/// Inserts a UserGoalEntity with sensible defaults.
@discardableResult
...
/// Inserts a SavingEntryEntity with the given date offset (negative = days ago).
@discardableResult
...
// MARK: - 2. PersistenceController – Initialisation
...
// MARK: - 3. UserGoalEntity – createOrUpdate
...
// MARK: - 4. UserGoalEntity – updateStreak
...
// MARK: - 5. SavingEntryEntity – create
...
final class BadgeEntityTests: XCTestCase {
    ...
    func test_seedAll_isIdempotent() {
        BadgeEntity.seedAll(in: ctx)
        try? ctx.save()
        BadgeEntity.seedAll(in: ctx)   // called twice
        try? ctx.save()

        let req = NSFetchRequest<BadgeEntity>(entityName: "BadgeEntity")
        let count1 = (try? ctx.count(for: req)) ?? 0

        BadgeEntity.seedAll(in: ctx)   // called a third time
        try? ctx.save()
...
final class DecisionEntryEntityTests: XCTestCase {
    ...
    func test_decisionEntry_defaultIsdSaveIsFalse() {
        let entry = DecisionEntryEntity(context: ctx)
        entry.id = UUID()
        // isdSave not explicitly set → should default to false
...
// MARK: - 10. Dashboard Logic (pure functions, no SwiftUI)

/// Mirrors the pure computed logic from DashboardView without importing SwiftUI.
